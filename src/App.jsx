...
export default function MenuCalendar() {
  const [appView, setAppView] = useState('login'); // 'login', 'partyList', 'createParty', 'partyDetail'
  ...
  // Load user and parties on mount
  useEffect(() => {
    checkLoggedInUser();
  }, []);

  // Reload parties when user logs in/out
  useEffect(() => {
    if (loggedInUser) {
      loadParties();
    }
  ...
  const checkLoggedInUser = async () => {
    try {
      ...
      if (token) {
        // Verify token is valid by trying to fetch parties
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (isSignUp) {
        result = await api.signup(loginForm.username, loginForm.password);
      } else {
        result = await api.login(loginForm.username, loginForm.password);
      }
  ...
  const saveParties = async (updatedParties) => {
    try {
      // Update each modified party via the API
      for (const [partyId, partyData] of Object.entries(updatedParties)) {
        if (parties[partyId]) {
          // Party already exists, update it
          await api.updateParty(partyId, partyData.selections);
        }
      }
  ...
  const handleSelectItem = async (item) => {
    ...
    try {
      ...
      await api.updateParty(currentPartyId, updatedParty.selections);
      setParties(updatedParties);
    } catch (error) {
  ...
  const resetPartyData = async () => {
    if (confirm('Are you sure you want to reset all selections for this party?')) {
      try {
        ...
        await api.updateParty(currentPartyId, {});
        setParties(updatedParties);
        alert('Party selections have been reset!');
      } catch (error) {
        console.error('Error resetting party:', error);
        alert('Failed to reset selections. Please try again.');
      }
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8">
          <div className="flex items-center justify-center gap-3 mb-8">
            ...
            <h1 className="text-2xl font-bold text-gray-800">Party Now</h1>
          </div>
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full py-2 text-indigo-600 hover:text-indigo-700 transition-colors text-sm"
            >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Logged in as: <span className="font-medium">{loggedInUser?.username}</span></p>
                </div>
              </div>
